...
(function ($, moment) {
	...
	var FlaterialDateTimePicker = function (htmlElement, config) {
		...
		var iCanUse = function () {
			arguments.__proto__ = new Array().__proto__;

			for (var i in arguments) if (arguments[i] == null || arguments[i] == undefined) return false;
			return true;
		};

		var isOriginalEvent = function (e) { return (e.hasOwnProperty('originalEvent') && e.originalEvent.isTrusted); };
		...
		var yearsColNbMax = 18;
		var selectedDate = new moment(new Date(), "DD-MM-YYYY HH:mm"), forMonthBlem = selectedDate.month(), today = new moment();
		var preparingTime = { h: null, m: null };

		if (iCanUse(plugin.config.minDate)) {
		...
		if (iCanUse(plugin.config.maxDate)) {
		...
		if (iCanUse(plugin.config.currentDate)) {
			...
			selectedDate.hour(today.hour()).minute(today.minute());
		} else plugin.config.currentDate = false;

		if (iCanUse(plugin.config.currentTime)) {
			selectedDate.hour(new moment(plugin.config.currentTime, "HH:mm").hour()).minute(new moment(plugin.config.currentTime, "HH:mm").minute());
		} else plugin.config.currentTime = false;
			...
			if (uiToSelect) {
				...
				if (map.hasOwnProperty(uiToSelect) && map[uiToSelect]) {
					...
					if (selectedUI === 'WEEKS') {
						...
						if (plugin.config.maxDate && plugin.config.maxDate.isSame(selectedDate, 'month') && plugin.config.maxDate.isSame(selectedDate, 'year') && plugin.config.maxDate.date() < selectedDate.date()) tmp = plugin.config.maxDate.date();
					...
					if (selectedUI === 'TIME') {
						...
						preparingTime.h = modulo(selectedDate.hour(), 12);
						preparingTime.m = modulo(selectedDate.minute(), 60);
		...
		var returnDate = function (date, format) {
			if (iCanUse(plugin.$htmlElement) && $(document).find(plugin.$htmlElement).length > 0) {
				plugin.$htmlElement.val(date.format(format));
			} else throw Error('Target HTML Element not found in DOM !');
		};
		...
		var updateTimeUI = function (callback) {
			...
			$timeHoursInput.val(twoDigits(TIME[0][parseInt(preparingTime.h - 1)]));
			$timeMinutesInput.val(twoDigits(TIME[1][parseInt(preparingTime.m)]));
		...
		var selectTimeInBase = function (hour, minute, callback) {
			if (!iCanUse(hour, minute)) throw Error('Invalid parameters given !');
			if (!callback) throw Error('Callback is required to execute this function !');
				...
				return new Array(3).fill([]).map(function (iElement, i) {
					if (i === 0) { /* Heures */
						return new Array(12).fill(1).map(function (jElement, j) {
							return twoDigits(j + jElement);
						});
					}

					if (i === 1) { /* Minutes */
						return new Array(60).fill(0).map(function (kElement, k) {
							return twoDigits(k + kElement);
						});
					}
			...
			for (var i = 0; i < weeksBase.length; i++) {
				...
				for (var j = 0; j < weeksBase[i].length; j++) {
					if (typeof weeksBase[i][j] === "number") {
						var date = twoDigits(weeksBase[i][j]), oClass = "";
						...
						var date = twoDigits(weeksBase[i][j].date);
		...
		var acceptUIEvents = function () {
			...
			if (UI() != "TIME") {
				...
				$(document).on('keydown', function (e) {
					if ((!$('input, textarea, div[contenteditable]').is(':focus')) && isOriginalEvent(e) && [TOP, BOTTOM, LEFT, RIGHT, ENTER].indexOf(e.keyCode) > -1) {
				...
				$timeHoursInput = $bigMaster.find('input[name=time-hours-input]');
				$timeMinutesInput = $bigMaster.find('input[name=time-minutes-input]');
				$timeCmdUp = $bigMaster.find('.time-cmd.time-cmd-up');
				$timeCmdDown = $bigMaster.find('.time-cmd.time-cmd-down');
				$timeAMPM = $bigMaster.find('li.time-value.time-ampm');
				...
				$timeCmdUp.off('click');
				$timeCmdUp.on('click', function (e) {
					if (isOriginalEvent(e)) if (UI() === "TIME") {
						if (timeComposant() === 0) {
							selectHourInBase(parseInt(preparingTime.h) + 1);
						}
						if (timeComposant() === 1) {
							selectMinInBase(parseInt(preparingTime.m) + 1);
						}
						if (timeComposant() === 2) {
							toggleMeridian();
						}
				...
				$timeCmdDown.off('click');
				$timeCmdDown.on('click', function (e) {
					if (isOriginalEvent(e)) if (UI() === "TIME") {
						if (timeComposant() === 0) {
							selectHourInBase(parseInt(preparingTime.h) - 1);
						}
				...
				$timeHoursInput.off('keyup');
				$timeHoursInput.on('keyup', function (e) {
					if (isOriginalEvent(e) && e.keyCode === ENTER) {
						$(this).trigger("focusout");
					}
				});
				$timeHoursInput.off('focusout');
				$timeHoursInput.on('focusout', function (e) {
					if (isOriginalEvent(e)) {
						if (parseInt($(this).val().trim()) > 12 ) meridian('PM');
						else meridian('AM');
						var response = selectHourInBase(parseInt($(this).val().trim())%12);
						$(this).val(response.toString());

						updateTimeUI(function () {});
					}
				});
				$timeHoursInput.off('focusin');
				$timeHoursInput.on('focusin', function (e) {
					$(this).get(0).select();
					if (isOriginalEvent(e)) timeComposant(0);
				});
				$timeHoursInput.off('mouseover');
				$timeHoursInput.on('mouseover', function (e) {
					$timeMinutesInput.blur();
					if (isOriginalEvent(e)) timeComposant(0);
				});
				$timeMinutesInput.off('keyup');
				$timeMinutesInput.on('keyup', function (e) {
					if (isOriginalEvent(e) && e.keyCode === ENTER) {
						$(this).trigger("focusout");
					}
				});
				$timeMinutesInput.off('focusin');
				$timeMinutesInput.on('focusin', function (e) {
					$(this).get(0).select();
					if (isOriginalEvent(e)) timeComposant(1);
				});
				$timeMinutesInput.off('mouseover');
				$timeMinutesInput.on('mouseover', function (e) {
					$timeHoursInput.blur();
					if (isOriginalEvent(e)) timeComposant(1);
				});
				$timeMinutesInput.off('focusout');
				$timeMinutesInput.on('focusout', function (e) {
					if (isOriginalEvent(e)) {
						var response = selectMinInBase(parseInt($(this).val().trim())%60);
						$(this).val(response.toString());
				...
				$timeAMPM.off('click');
				$timeAMPM.on('click', function (e) {
					if (isOriginalEvent(e)) timeComposant(2);
				});
				$timeAMPM.off('mouseover');
				$timeAMPM.on('mouseover', function (e) {
					$timeMinutesInput.blur();
					$timeHoursInput.blur();
					if (isOriginalEvent(e)) timeComposant(2);
				});
		...
		var showInUI = function (date) {
			...
			$dayInt.text(twoDigits(date.date()));

			$hours.text(twoDigits(date.hour()));
			$mins.text(twoDigits(date.minute()));
		};
		...
		var initEvents = function () {
			...
			$time.on('click', function (e) {
				if (isOriginalEvent(e)) if (!(UI() === "TIME")) UI('TIME');
			});
			$ok.on('click', function (e) {
				if (isOriginalEvent(e)) returnDate(selectedDate, plugin.config.format);
			});
